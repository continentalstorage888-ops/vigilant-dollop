...
const AppContent: React.FC = () => {
    ...
    const { 
        ...
        showConfetti,
        setShowConfetti,
        isPetDoingBackflip, // New state for pet
        menuBarClickCount,
    ...
    // Easter Egg: General Click Counter (existing)
    useEffect(() => {
        if (clickCount === 100) {
    ...
    // Easter Egg: Menu Bar Name Click (new)
    useEffect(() => {
        if (menuBarClickCount >= 10 && !secretGameUnlocked) {
            ...
            addNotification({
                ...
                message: 'You found the hidden Tic-Tac-Toe game! Enjoy!'
            });
            toast('You found a secret game!', 'success');
            resetMenuBarClickCount(); // Reset after unlocking
        }
    ...
    const appComponents: { [key in AppID]: React.FC } = {
        ...
        case_studies: CaseStudiesApp,
        data_wave_dashboard: DataWaveIndiaDashboard,
        astro_viewer: AstroViewerApp,
        astro_tracker: AstroTrackerApp,
    };
    ...
    return (
        <div onClick={incrementClickCount} className={`relative w-screen h-screen overflow-hidden transition-colors duration-500 ${`wallpaper-${wallpaper}-${theme}`} select-none`}>
            ...
            <Desktop />
            <DesktopPet isDoingBackflip={isPetDoingBackflip} /> {/* Pass new prop */}
                    ...
                    {Object.values(windows).map((win) => {
                        ...
                        return (
                            <Window
                                ...
                                id={win.id}
                                winState={win}
                                padding={!['terminal', 'business_card', 'editor', 'data_wave_dashboard', 'astro_viewer', 'astro_tracker'].includes(win.id)}
                            >
                                <AppContentComponent />
                            </Window>
                        );
            ...
            <AnimatePresence>
                {showMatrixRain && (
                    <MatrixRainOverlay key="matrix-rain" onClose={() => setShowMatrixRain(false)} />
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {showConfetti && (
                    <ReactConfetti
                        ...
                        recycle={false} // Only explode once
                        numberOfPieces={500}
                        gravity={0.5}
                        initialVelocityY={20}
                        tweenDuration={10000}
            ...
            {/* Demo Simulation Component - Can be easily removed */}
            <DemoSimulation />
        </div>
    );
};
