...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const openWindow = useCallback((id: AppID, title?: string) => {
        setZIndexCounter(prevCounter => {
            ...
            if (appInfo && appInfo.id !== 'editor' && appInfo.id !== 'terminal' && appInfo.id !== 'hire_me') { // Don't notify for editor, terminal or hire_me opening
                addNotification({
                    icon: appInfo.icon,
    ...
    const incrementMenuBarClickCount = () => {
        setMenuBarClickCount(prev => prev + 1);
    };

    const resetMenuBarClickCount = () => {
        setMenuBarClickCount(0);
    };

    const unlockSecretGame = () => setSecretGameUnlocked(true);
