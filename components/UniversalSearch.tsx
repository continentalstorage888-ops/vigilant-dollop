...
import { Search, CornerDownLeft, FileText, Download, ExternalLink } from 'lucide-react';
        ...
        caseStudies.forEach(caseStudy => {
            if (caseStudy.title.toLowerCase().includes(lowerQuery)) {
                allResults.push({
                    ...
                    action: () => openWindow('case_studies'),
                });
            }
        });
        ...
        mediumArticles.forEach(article => {
            if (article.title.toLowerCase().includes(lowerQuery)) {
                allResults.push({
                    ...
                    title: article.title,
                    icon: ExternalLink,
                    action: () => window.open(article.link, '_blank'),
                });
            }
        });
                    ...
                    {results.length > 0 ? (
                        ...
                        demoSearchQuery.trim() && (
                            <div className="p-8 text-center text-stone-500">
                                <p>No results found for "{demoSearchQuery}"</p>
                            </div>
                        )
                    )}
                </div>
