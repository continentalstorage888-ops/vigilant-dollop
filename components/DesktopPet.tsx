...
const DesktopPet: React.FC<DesktopPetProps> = ({ isDoingBackflip }) => {
  ...
  const [desktopBounds, setDesktopBounds] = useState(() => ({ top: 50, left: 50, right: window.innerWidth - 150, bottom: window.innerHeight - 250 }));
  ...
  const handlePetClick = (e: MouseEvent | PointerEvent) => {
    e.stopPropagation();
    ...
    if (clickCountRef.current === 3) {
        if (!isGuiding && moodRef.current !== 'backflipping') {
            ...
            }, 1000); // Backflip animation duration
        }
        clickCountRef.current = 0; // Reset
    } else {
        clickTimerRef.current = window.setTimeout(() => {
            clickCountRef.current = 0; // Reset if not a triple click within time
            handleTap(e); // Proceed with normal tap action if not triple click
        }, 300); // Time window for triple click
    }
  };
  ...
  useEffect(() => {
    const handleResize = () => {
        setDesktopBounds({ top: 50, left: 50, right: window.innerWidth - 150, bottom: window.innerHeight - 250 });
        setTourSteps([
            { x: window.innerWidth / 2 - 60, y: window.innerHeight / 2 - 100, message: "Welcome! I'm your AI guide. Let's explore the Portfolio OS together. Click 'Next' to continue." },
            { x: 150, y: 150, message: "This is your desktop. You can open apps by double-clicking the icons here." },
            { x: window.innerWidth / 2 - 60, y: window.innerHeight - 200, message: "Down here is the dock. It holds your most-used applications for quick access." },
  ...
  const showMessage = useCallback((newMessage: string, duration = 4000) => {
    if (isGuiding) return; // Don't show random messages during tour
  ...
  useEffect(() => {
    const decideNextAction = () => {
        ...
        if (nextAction > 0.3) { // 70% chance to walk
            setMood('walking');
            ...
            const currentPos = catRef.current?.getBoundingClientRect();
            setDirection(targetX > (currentPos?.x || 0) ? 'right' : 'left');
            
            controls.start({ x: targetX, y: targetY, transition: { duration: 5 + Math.random() * 5, ease: 'linear' } }).then(() => {
                if (moodRef.current === 'walking') {
                    setMood('idle');
                }
            });
        } else if (nextAction > 0.1) { // 20% chance to sleep
            setMood('sleepy');
            setTimeout(() => setMood('sleeping'), 2000);
        } else { // 10% chance to just stay idle
            setMood('idle');
        }
    };
  ...
  }, [mood, controls, isGuiding, isDragging, desktopBounds]);
  ...
  useEffect(() => {
    ...
    const handleTap = (e: MouseEvent | PointerEvent) => {
        if (isGuiding || mood === 'backflipping') return;
        e.stopPropagation();
  ...
  }, [windows, clickCount, isIdle, showMessage, controls, isGuiding]);
  ...
  // Eye following logic
  useEffect(() => {
    const canFollowMouse = ['idle', 'happy', 'curious'].includes(mood) && !isDragging;
    ...
    if (canFollowMouse) {
        const handleMouseMove = (e: MouseEvent) => {
            if (catRef.current) {
                ...
                const maxOffset = 2; // How far the pupil can move
  ...
  const handleFeed = () => {
    if (actionTimeoutRef.current) clearTimeout(actionTimeoutRef.current);
    controls.stop();
    const happinessBoost = Math.floor(Math.random() * 3) + 2; // 2, 3, or 4
  ...
  // Guided Tour Logic
    ...
    useEffect(() => {
        if (isGuiding && guideStep > 0) {
            ...
            const currentPos = catRef.current?.getBoundingClientRect();
            setDirection(currentStep.x > (currentPos?.x || 0) ? 'right' : 'left');
  ...
  const renderEyes = () => {
    ...
    } else if (mood === 'happy' || mood === 'celebrating' || mood === 'playful' || mood === 'backflipping') {
        return (
             <g>
                <path d="M 45 46 Q 50 52 55 46" stroke="#1c1917" strokeWidth="2" fill="none" strokeLinecap="round" />
                <path d="M 65 46 Q 70 52 75 46" stroke="#1c1917" strokeWidth="2" fill="none" strokeLinecap="round" />
            </g>
        )
    } else {
      return (
        <g>
          ...
          <ellipse cx={51 + pupilOffset.x} cy={47 + pupilOffset.y} rx="2" ry="3" fill="black" />
          <ellipse cx="70" cy="48" rx="5" ry="7" fill="white" />
          <ellipse cx={71 + pupilOffset.x} cy={47 + pupilOffset.y} rx="2" ry="3" fill="black" />
        </g>
      );
    }
  };
